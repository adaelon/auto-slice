...
const sliceSpecPath = path.join(repoRoot, "contracts", "slices", "S07.json");
const receiptRelativePath = "artifacts/s07/completion-receipt.json";
const virtualTraceRelativePath = "artifacts/s07/virtual-clock-event-trace.json";
const raceReportRelativePath = "artifacts/s07/race-repeat-report.json";
const observabilityMatrixRelativePath = "artifacts/s07/observability-failure-matrix.json";
...
function isS07OwnedPath(relativePath) {
  const exactPaths = new Set([
    "contracts/slices/S07.json",
    "docs/代码链路.md",
    "docs/架构.md",
    "package.json",
    "scripts/verify-s07-evidence.mjs",
    "scripts/verify-s07.mjs",
    "test/compaction-monitor.test.ts",
    "test/helpers/compaction-race-worker.ts",
  ]);
  const prefixes = ["artifacts/s07/", "src/controller/compaction-monitor/"];
...
function validateSliceSpec(sliceSpec) {
  if (
    ...
    throw new Error("contracts/slices/S07.json is not the expected SliceSpec v1 requiring S02.");
  }
  if (!Array.isArray(sliceSpec.inputs) || !Array.isArray(sliceSpec.checks)) {
    throw new Error("S07 SliceSpec must contain inputs and checks arrays.");
  }
  if (!sliceSpec.failure_codes?.includes("compaction_observability_unavailable")) {
    throw new Error("S07 must freeze compaction_observability_unavailable.");
  }
  for (const input of sliceSpec.inputs) {
    if (typeof input?.path !== "string" || typeof input.digest !== "string") {
      throw new Error("S07 input entries must contain path and digest strings.");
    }
    const inputPath = path.join(repoRoot, input.path);
    if (!existsSync(inputPath) || sha256File(inputPath) !== input.digest) {
      throw new Error(`S07 input digest changed for ${input.path}.`);
    }
  }
  ...
  for (const output of receipt.output_digests) {
    if (typeof output?.path === "string" && isS07OwnedPath(output.path)) {
      continue;
    }
    if (
  ...
  const expectedChecks = ["build", "typecheck", "test", "s07_evidence", "lint"];
  if (JSON.stringify(sliceSpec.checks.map((check) => check?.id)) !== JSON.stringify(expectedChecks)) {
    throw new Error("S07 deterministic check order changed.");
  }
}
...
function runChecks(sliceSpec) {
  ...
  for (const check of sliceSpec.checks) {
    if (
      ...
      throw new Error("S07 contains an invalid CheckSpec.");
    }
  ...
  if (failed) {
    throw new Error("One or more S07 deterministic checks failed.");
  }
  return { receipts, outputs };
}

function parseEvidence(outputs) {
  const serialized = outputs.get("s07_evidence");
  if (typeof serialized !== "string" || serialized.length === 0) {
    throw new Error("The s07_evidence check produced no JSON evidence.");
  }
  const evidence = JSON.parse(serialized);
  if (
    evidence?.virtual_clock_event_trace?.result !== "PASS" ||
    evidence?.race_repeat_report?.result !== "PASS" ||
    evidence?.observability_failure_matrix?.result !== "PASS" ||
    evidence.race_repeat_report.iterations !== 12 ||
    evidence.race_repeat_report.one_transition_per_iteration !== true
  ) {
    throw new Error("The s07_evidence check returned an invalid or failing report.");
  }
  return evidence;
}
...
function assertEvidenceMatches(relativePath, expected) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(`${relativePath} no longer matches deterministic S07 evidence.`);
  }
}
...
function validateExpectedArtifacts(sliceSpec, includeReceipt = true) {
  ...
  if (missing.length > 0) {
    throw new Error(`S07 expected artifacts are missing: ${missing.join(", ")}`);
  }
}

function verifyCommittedEvidence(sliceSpec, checkReceipts, evidence) {
  assertEvidenceMatches(virtualTraceRelativePath, evidence.virtual_clock_event_trace);
  assertEvidenceMatches(raceReportRelativePath, evidence.race_repeat_report);
  assertEvidenceMatches(observabilityMatrixRelativePath, evidence.observability_failure_matrix);
  validateExpectedArtifacts(sliceSpec);
  const receipt = parseJsonFile(path.join(repoRoot, receiptRelativePath));
  if (
    receipt?.slice_id !== "S07" ||
    receipt.contract_digest !== sha256File(sliceSpecPath) ||
    typeof receipt.owned_diff_digest !== "string"
  ) {
    throw new Error("The committed S07 CompletionReceipt is inconsistent with its SliceSpec.");
  }
  ...
  if (JSON.stringify(committedCheckIds) !== JSON.stringify(expectedCheckIds)) {
    throw new Error("The committed S07 CompletionReceipt has a different check set.");
  }
  for (const output of receipt.output_digests ?? []) {
    if (
      typeof output?.path !== "string" ||
      ...
      throw new Error(`Committed S07 output changed: ${String(output?.path)}.`);
    }
  }
  ...
  if (expectedOwnedDiffDigest !== receipt.owned_diff_digest) {
    throw new Error("The committed S07 owned diff digest is invalid.");
  }
  const dirtyS07Paths = collectTouchedPaths("HEAD").filter(
    (entry) => entry !== "SESSION_CHECKPOINT.md" && isS07OwnedPath(entry),
  );
  if (dirtyS07Paths.length > 0) {
    throw new Error(`S07-owned paths changed after its receipt was committed: ${dirtyS07Paths.join(", ")}`);
  }
  process.stdout.write(`S07 CompletionReceipt verified: ${receiptRelativePath}\n`);
...
async function main() {
  ...
  writeJsonAtomic(virtualTraceRelativePath, evidence.virtual_clock_event_trace);
  writeJsonAtomic(raceReportRelativePath, evidence.race_repeat_report);
  writeJsonAtomic(observabilityMatrixRelativePath, evidence.observability_failure_matrix);
  ...
  const unownedPaths = touchedBeforeReceipt.filter((entry) => !isS07OwnedPath(entry));
  if (unownedPaths.length > 0) {
    throw new Error(`S07 touched paths outside its ownership: ${unownedPaths.join(", ")}`);
  }
  ...
  validateExpectedArtifacts(sliceSpec);
  process.stdout.write(`S07 CompletionReceipt: ${receiptRelativePath}\n`);
