...
const sliceSpecPath = path.join(repoRoot, "contracts", "slices", "S08.json");
const receiptRelativePath = "artifacts/s08/completion-receipt.json";
const adapterReportRelativePath = "artifacts/s08/thread-adapter-contract-report.json";
const lateWriteReportRelativePath = "artifacts/s08/late-write-report.json";
const failureMatrixRelativePath = "artifacts/s08/failure-closure-matrix.json";
...
function isS08OwnedPath(relativePath) {
  const exactPaths = new Set([
    "contracts/slices/S08.json",
    "docs/代码链路.md",
    "docs/架构.md",
    "package.json",
    "scripts/verify-s08-evidence.mjs",
    "scripts/verify-s08.mjs",
    "test/thread-control.test.ts",
  ]);
  const prefixes = ["artifacts/s08/", "src/controller/thread-control/"];
...
function validatePrerequisiteReceipt(sliceId, relativePath) {
  ...
  for (const output of receipt.output_digests) {
    if (typeof output?.path === "string" && isS08OwnedPath(output.path)) {
      continue;
    }
    if (
...
function validateSliceSpec(sliceSpec) {
  if (
    ...
    JSON.stringify(sliceSpec.requires) !== JSON.stringify(["S03", "S07"])
  ) {
    throw new Error("contracts/slices/S08.json is not SliceSpec v1 requiring S03 and S07.");
  }
  if (!Array.isArray(sliceSpec.inputs) || !Array.isArray(sliceSpec.checks)) {
    throw new Error("S08 SliceSpec must contain inputs and checks arrays.");
  }
  if (!sliceSpec.failure_codes?.includes("source_interrupt_failed")) {
    throw new Error("S08 must freeze source_interrupt_failed.");
  }
  for (const input of sliceSpec.inputs) {
    if (typeof input?.path !== "string" || typeof input.digest !== "string") {
      throw new Error("S08 input entries must contain path and digest strings.");
    }
    const inputPath = path.join(repoRoot, input.path);
    if (!existsSync(inputPath) || sha256File(inputPath) !== input.digest) {
      throw new Error(`S08 input digest changed for ${input.path}.`);
    }
  }
  validatePrerequisiteReceipt("S03", "artifacts/s03/completion-receipt.json");
  validatePrerequisiteReceipt("S07", "artifacts/s07/completion-receipt.json");
  const expectedChecks = ["build", "typecheck", "test", "s08_evidence", "lint"];
  if (JSON.stringify(sliceSpec.checks.map((check) => check?.id)) !== JSON.stringify(expectedChecks)) {
    throw new Error("S08 deterministic check order changed.");
  }
}
...
function runChecks(sliceSpec) {
  ...
  for (const check of sliceSpec.checks) {
    if (
      ...
      throw new Error("S08 contains an invalid CheckSpec.");
    }
  ...
  if (failed) {
    throw new Error("One or more S08 deterministic checks failed.");
  }
  return { receipts, outputs };
}

function parseEvidence(outputs) {
  const serialized = outputs.get("s08_evidence");
  if (typeof serialized !== "string" || serialized.length === 0) {
    throw new Error("The s08_evidence check produced no JSON evidence.");
  }
  const evidence = JSON.parse(serialized);
  if (
    evidence?.thread_adapter_contract_report?.result !== "PASS" ||
    evidence?.late_write_report?.result !== "PASS" ||
    evidence?.failure_closure_matrix?.result !== "PASS" ||
    evidence.thread_adapter_contract_report.interrupt_side_effects !== 1 ||
    evidence.late_write_report.freeze_observed_before_interrupt !== true ||
    evidence.failure_closure_matrix.scenarios?.some(
      (entry) => entry.final_status !== "NEEDS_USER" || entry.epoch_rotated !== false,
    )
  ) {
    throw new Error("The s08_evidence check returned an invalid or failing report.");
  }
  return evidence;
}
...
function assertEvidenceMatches(relativePath, expected) {
  ...
  if (JSON.stringify(actual) !== JSON.stringify(expected)) {
    throw new Error(`${relativePath} no longer matches deterministic S08 evidence.`);
  }
}
...
function validateExpectedArtifacts(sliceSpec, includeReceipt = true) {
  ...
  if (missing.length > 0) {
    throw new Error(`S08 expected artifacts are missing: ${missing.join(", ")}`);
  }
}

function verifyCommittedEvidence(sliceSpec, checkReceipts, evidence) {
  assertEvidenceMatches(adapterReportRelativePath, evidence.thread_adapter_contract_report);
  assertEvidenceMatches(lateWriteReportRelativePath, evidence.late_write_report);
  ...
  if (
    receipt?.slice_id !== "S08" ||
    receipt.contract_digest !== sha256File(sliceSpecPath) ||
    typeof receipt.owned_diff_digest !== "string"
  ) {
    throw new Error("The committed S08 CompletionReceipt is inconsistent with its SliceSpec.");
  }
  ...
  if (JSON.stringify(committedCheckIds) !== JSON.stringify(expectedCheckIds)) {
    throw new Error("The committed S08 CompletionReceipt has a different check set.");
  }
  for (const output of receipt.output_digests ?? []) {
    if (
      typeof output?.path !== "string" ||
      ...
      throw new Error(`Committed S08 output changed: ${String(output?.path)}.`);
    }
  }
  ...
  if (expectedOwnedDiffDigest !== receipt.owned_diff_digest) {
    throw new Error("The committed S08 owned diff digest is invalid.");
  }
  const dirtyS08Paths = collectTouchedPaths("HEAD").filter(
    (entry) => entry !== "SESSION_CHECKPOINT.md" && isS08OwnedPath(entry),
  );
  if (dirtyS08Paths.length > 0) {
    throw new Error(`S08-owned paths changed after its receipt was committed: ${dirtyS08Paths.join(", ")}`);
  }
  process.stdout.write(`S08 CompletionReceipt verified: ${receiptRelativePath}\n`);
...
async function main() {
  ...
  writeJsonAtomic(adapterReportRelativePath, evidence.thread_adapter_contract_report);
  writeJsonAtomic(lateWriteReportRelativePath, evidence.late_write_report);
  ...
  const unownedPaths = touchedBeforeReceipt.filter((entry) => !isS08OwnedPath(entry));
  if (unownedPaths.length > 0) {
    throw new Error(`S08 touched paths outside its ownership: ${unownedPaths.join(", ")}`);
  }
  ...
  validateExpectedArtifacts(sliceSpec);
  process.stdout.write(`S08 CompletionReceipt: ${receiptRelativePath}\n`);
