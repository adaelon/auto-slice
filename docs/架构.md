...
CompressionHandoffCoordinator
  ...
  -> Handoff receipt 与 Evidence Index 共同验证该 canonical revision
```
...
awaitHandoff
  -> terminal private commandExecution projection
  -> exact direct prepare(Source UUID, mode, two paths)
  -> exact direct publish(same helper, same workDir)
  -> reread two regular non-link files and compare raw-byte digests
  -> spawn(shell=false) helper verify-evidence and bind Source revision
  -> HandoffReceiptV2 + canonical artifact_digest
  -> journal COMPLETED
...
Receipt builder 不读取 `agentMessage`、模型 final reply 或 `CompressionRequest.prompt`；旧 `HandoffReceipt(workflow_version:"v2")` 只保留给 S21 迁移兼容。路径/字节/Source revision/consumer contract 任一不一致均在 Continuation 前失败闭合。

## S21 真实 Continuation launcher 数据流

```text
ContinuationCoordinator.createEnvelope
  -> V2: Source/Compression task + Compression turn + schema v2
  -> legacy replay: Source/Compression task，省略 turn/schema 字段
  -> verified Handoff paths/digests + consumer contract + rotated write epoch

AppServerContinuationTaskLauncher.start
  -> lstat/realpath/read Handoff Markdown，先复核 regular file、UTF-8、大小与 digest
...
Launcher 结构化错误码由 Coordinator 映射到既有失败原因：Handoff 字节错误为 `handoff_integrity_failed`；Ready 证据、epoch、第二 Turn start 与 Progress 失败分别为 `consumer_contract_violated`、`write_epoch_mismatch`、`grant_call_failed`、`progress_call_failed`。legacy receipt 只保留迁移重放能力，S20 以后新的生产输入始终是 `HandoffReceiptV2`。
