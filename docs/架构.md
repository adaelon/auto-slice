...
默认 composition root 以一个 Host-owned App Server client 共享承载 Development Task、Source Thread 控制和 fresh-task session 基座；普通路径不构造或调用 Git/产物验收组件。S19 只提供 fresh root、顺序 Turn 与私有事件投影，Compression/Continuation launcher 在 S20/S21 前仍须显式注入；未注入时压缩超时路径确定性失败闭合，不会伪造 Handoff 或扩大写权限。
