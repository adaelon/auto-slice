...
flowchart LR
  ...
  AppServerTaskHost --> FreshTaskSessions["CodexAppServerFreshTaskSessions"]
  AppServerTaskHost --> AppServerClient["single Host-owned CodexAppServerClient"]
  AppServerTaskHost --> CompressionLauncher
  AppServerTaskHost --> AppServerContinuationLauncher["AppServerContinuationTaskLauncher"]
  AppServerDevelopmentTask --> AppServerClient
  FreshTaskSessions --> AppServerClient
  ...
  AppServerDevelopmentTask --> AppServerProtocolV2["AppServerProtocolV2 fail-closed codecs"]
  ...
  AppServerClient --> PrivateEventRouter["Fresh-task private event router"]
  PrivateEventRouter --> FreshTaskSessions
  ...
  ContinuationLauncher --> AppServerContinuationLauncher
  AppServerContinuationLauncher --> FreshTaskSessions
  AppServerContinuationLauncher --> ContinuationTask["Fresh Continuation Task"]
  CompressionLauncher --> FreshTaskSessions
  ...
  S17Verifier --> AppServerProtocolV2
  S17Verifier --> S17Fixture["Minimal checked-in schema projection"]
  AppServerProtocolV2 -. "供 S19–S21 使用" .-> FreshTaskLaunchers["Fresh-task / Handoff launchers"]
...
默认 composition root 以一个 Host-owned App Server client 共享承载 Development Task、Source Thread 控制和 fresh-task session 基座；普通路径不构造或调用 Git/产物验收组件。S20/S21 已分别将真实 `AppServerCompressionTaskLauncher` 与 `AppServerContinuationTaskLauncher` 接入默认 Host；两者只从私有机器事件和验证后的文件字节构造回执，不从模型 final reply 取字段。
...
S17 的 codec 已由 S18 接入 summary-only `thread/read`；JSON Schema 默认值仍只用于漂移诊断，发送方必填性以本机生成 TypeScript 为准。

## S18 Export-owned revision 迁移

```text
CompactionMonitor
  -> 新 compaction 持久化 source_interruption_schema_version=2

SourceInterruptionCoordinator
  -> freeze Project Write Lease
  -> marker 缺失：NEEDS_USER(source_interruption_migration_required)，不改旧 effect receipt
  -> ThreadControl.interrupt：turn/interrupt 后只接受同 thread/turn interrupted terminal
  -> ThreadControl.inspect：thread/read(includeTurns=false)
       接受 persistent + turns=[]；拒绝非空 turns、任意 items 与 observed delete
  -> effect receipt 完成 + epoch rotate + HANDOFF_EXPORTING

CompressionHandoffCoordinator
  -> CompressionRequest 不携带 Source revision
  -> export-codex-handoff 对 rollout 原始字节签发 sha256 SourceEvidenceRevision
  -> schema 3 receipt 不携带 revision；只绑定 completed Compression Turn 选择的首个 Markdown 地址
```

S15 的 opaque revision provider 路径只保留在历史文档和兼容 fixture；`src/` 不再定义或读取 Host revision token。Archive/closed notification 不伪造负事实，持久可读性由真实 summary read 裁决；明确 delete 仍失败闭合。

## S19 Fresh-task App Server session 基座

```text
CodexAppServerTaskHost
  -> 创建一个 CodexAppServerClient
  -> CodexAppServerDevelopmentTask 借用该 client，不取得 dispose ownership
  -> CodexAppServerFreshTaskSessions 附着唯一 private router

freshTaskSessions.start(kind, source_thread_id, cwd)
  -> thread/start（compression=workspace-write；continuation=read-only；ephemeral=false）
  -> AppServerProtocolV2 解码
  -> 只接受 canonical UUID + sessionId=id + parent/fork=null + turns=[]
  -> 拒绝 Source/既有 fresh root 身份复用

session.startTurn(request)
  -> READY|TURN_TERMINAL -> TURN_STARTING -> TURN_ACTIVE
  -> 同 thread 最多一个 active Turn
  -> item/completed：按 thread/turn 注册表认领，仅保存 allowlist 且受 item/count/turn bytes 上限约束
  -> turn/completed：只投影身份、terminal status 与完成时间，忽略 terminal items 正文
  -> TURN_TERMINAL 后才允许下一个 turn/start

raw notification
  -> private router HANDLED：只进入对应 fresh session
  -> UNHANDLED：交给既有 HostEventFirewall 重建 ControllerSignal
  -> 交叉 thread/turn、迟到 item、重复 terminal 或超预算：无正文协议错误并关闭连接

TaskHost.dispose
  -> Development Task 取消订阅 -> private sessions 关闭 -> shared client 只 dispose 一次
```

S19 不解析 export skill、不构造 Handoff/Ready receipt、不实现两个生产 launcher，也不触碰 Project Write Lease；这些能力分别留给 S20/S21。

## S20 真实 Compression launcher 数据流

```text
CompressionHandoffCoordinator.createRequest
  -> run_id + slice_id + Source UUID + exact model policy + effect key

AppServerCompressionTaskLauncher.start
  -> skills/list(SourceCwd, forceReload=true)
  -> journal ALLOCATED(effect key, attempt_number, two target paths)
  -> create and verify Handoff Artifact Root/Run/Slice/attempt (no link, no overlap)
  -> fresh_task_sessions.start(kind=compression)
  -> turn/start(text + skill, workspaceWrite only for artifactRoot, network=false)
  -> journal LAUNCHED(thread_id, turn_id, launch receipt)

awaitHandoff
  -> Compression Turn completed terminal（不投影 helper commandExecution）
  -> 从 bounded 最终 agentMessage 提取第一个绝对本地 Markdown 地址
  -> 绑定 Source/Compression task、Compression turn、地址
  -> HandoffResultReceipt(schema 3) + canonical artifact_digest
  -> journal COMPLETED

restart/retry
  -> COMPLETED: replay persisted launch/receipt without App Server calls
  -> ALLOCATED|LAUNCHED|FAILED: persist attempt_number+1 before touching a new directory
  -> old attempt bytes and managed failure workDir remain diagnostic-only
```

Receipt builder 只读取 completed Compression Turn 的 bounded 最终 `agentMessage`，不读取 helper command/output、Evidence Index、`HANDOFF_VERIFY`、Worker Content 或 `CompressionRequest.prompt`。schema 2 `HandoffReceiptV2` 与旧 `HandoffReceipt(workflow_version:"v2")` 只保留迁移重放；schema 3 是新生产输入。该边界由 ADR-0011 §5 的 S22 裁决取代 S20 发布时的 prepare→publish 命令证明。

## S21 真实 Continuation launcher 数据流

```text
ContinuationCoordinator.createEnvelope
  -> schema 3: Source/Compression task + Compression turn + selected Markdown path
  -> schema 2: historical paths/digests + consumer contract
  -> legacy replay: Source/Compression task，省略 turn/schema 字段
  -> rotated write epoch

AppServerContinuationTaskLauncher.start
  -> schema 3：读取 selected path 的非空、bounded、无 NUL UTF-8 Markdown
  -> schema 2/legacy：额外复核 regular non-link file、realpath 与 digest
  -> fresh_task_sessions.start(kind=continuation)，新 UUID 不复用 Source/Compression
  -> turn/start(goal text + bounded Handoff text, readOnly, gpt-5.6-sol/max)

awaitReady
  -> 等待 completed terminal
  -> 私有 completed items 按顺序扫描
  -> 任一 command/fileChange/MCP/web/collab item 拒绝
  -> 第一份非空 agentMessage -> draft digest + bounded rollout digest
  -> ReadyReceipt(write_access=false, evidence reads=0)

grantWrite + awaitProgress
  -> 仅 READY phase 且 newWriteEpoch == ResumeEnvelope.write_epoch
  -> 同 fresh root 第二个 turn/start(workspaceWrite, network=false, zero content projection)
  -> LeaseReceipt 绑定 task/lease/epoch/workspace
  -> completed terminal -> ProgressReceipt(terminal projection digest)
  -> grant 后任一失败：ContinuationCoordinator 冻结 rotated epoch 并进入 NEEDS_USER
```

Launcher 结构化错误码由 Coordinator 映射到既有失败原因：Handoff 路径读取错误为 `handoff_integrity_failed`；Ready 证据、epoch、第二 Turn start 与 Progress 失败分别为 `consumer_contract_violated`、`write_epoch_mismatch`、`grant_call_failed`、`progress_call_failed`。schema 2 与 legacy receipt 只保留迁移重放能力，S22 后新的生产输入始终是 `HandoffResultReceipt` schema 3。

## S22 默认生产链验收数据流

```text
hermetic gate（默认 Host + fake App Server + 真实 fixture helper）
  -> 真实 30 秒 Compaction Timeout
  -> Source interrupt/read
  -> fresh Compression root/Turn completed
  -> completed Turn 最终结果的首个 Markdown 地址
  -> schema 3 path-only receipt（Evidence Index ignored；Host verify calls=0）
  -> fresh Continuation readOnly/workspaceWrite 双 Turn
  -> HERMETIC_CHAIN_PASS

live gate（本机 codex-cli 0.146.0 + disposable workspace/Source）
  -> Source readiness 独立 120 秒预算
  -> 从 SOURCE_INTERRUPT 起 1200 秒完整接力预算
  -> 同一默认 Host composition 与 path-only receipt 边界
  -> disposable PASS artifact + LIVE_CHAIN_PASS

unlock
  -> 两层 PASS + 三 UUID distinct + Controller canary hits=0
  -> default production chain unlocked
```

S22 的协议轨迹只记录 Source、Compression 与 Continuation 的有界控制面请求；prepare、Frame、MAP、REDUCE、publish 命令不进入 receipt、Controller trace 或 release report。provider timing、worker capacity 或 1200 秒预算失败只发布 blocker，不降级成 PASS。
...
- S15 历史 Host opaque revision 与 summary-only inspection：[ADR-0009](adr/0009-content-blind-controller.md)、`contracts/slices/S15.json`；生产路径已由 S18 取代
...
- S18 revisionless Source interruption 与 export-owned revision：[ADR-0011](adr/0011-export-owned-revision-and-fresh-task-launchers.md)、`src/controller/thread-control/source-interruption-coordinator.ts`、`src/controller/production/codex-app-server-development-task.ts`、`src/controller/handoff/compression-handoff-coordinator.ts`、`contracts/slices/S18.json`
- S19 shared client、fresh root、顺序 Turn 与私有事件路由：[ADR-0011](adr/0011-export-owned-revision-and-fresh-task-launchers.md)、[三任务接力切片方案](切片方案-App-Server三任务接力解锁.md)、`src/controller/production/app-server-fresh-task-session.ts`、`contracts/slices/S19.json`
- S20 默认 Compression launcher、持久 attempt 与 HandoffReceiptV2：[ADR-0011](adr/0011-export-owned-revision-and-fresh-task-launchers.md)、[三任务接力切片方案](切片方案-App-Server三任务接力解锁.md)、`src/controller/handoff/app-server-compression-task-launcher.ts`、`contracts/slices/S20.json`
- S21 默认 Continuation 双 Turn、Ready/epoch 门禁与 V2/legacy 迁移：[ADR-0011](adr/0011-export-owned-revision-and-fresh-task-launchers.md)、[三任务接力切片方案](切片方案-App-Server三任务接力解锁.md)、`src/controller/continuation/app-server-continuation-launcher.ts`、`contracts/slices/S21.json`
- S22 默认三任务 hermetic/live 双门禁与最终文件 receipt：[ADR-0011 §5](adr/0011-export-owned-revision-and-fresh-task-launchers.md)、[三任务接力切片方案](切片方案-App-Server三任务接力解锁.md)、`scripts/verify-s22.mjs`、`contracts/slices/S22.json`
