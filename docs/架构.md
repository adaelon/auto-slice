...
flowchart LR
  ...
  AppServerProtocolV2 -. "供 S19–S21 使用" .-> FutureHandoffLaunchers["Fresh-task / Handoff launchers"]
...
默认 composition root 以一个 Host-owned App Server client 共享承载 Development Task、Source Thread 控制和 fresh-task session 基座；普通路径不构造或调用 Git/产物验收组件。S20 已将真实 `AppServerCompressionTaskLauncher` 接入默认 Host；Continuation launcher 在 S21 前仍保持显式 fail-closed，占位路径不会伪造 Ready/Progress receipt 或扩大写权限。
